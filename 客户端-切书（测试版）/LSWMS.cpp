...
void LSWMS::drawLSegs(cv::Mat &img, std::vector<LSEG> &lSegs, std::vector<double> &errors, int thickness)
{
	...
	for(unsigned int i=0; i<lSegs.size(); i++)	
	{
		if((errors[i]/CV_PI)*180.0 < 45)  // 5º
			cv::line(img, lSegs[i][0], lSegs[i][1], colors[0], 1);
		else if((errors[i]/CV_PI)*180.0 < 90) // 10º
			cv::line(img, lSegs[i][0], lSegs[i][1], colors[1], 1);
		else if((errors[i]/CV_PI)*180.0 < 135) // 15º
